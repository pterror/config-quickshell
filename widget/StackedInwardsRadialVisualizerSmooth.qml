		...
		Shape {
			...
			Repeater {
				...
				Item {
					...
					property real outerVal: shape.index === 0 ? 0 : root.cumulative(shape.index - 1, modelData % root.count)
					property real innerVal: root.cumulative(shape.index, modelData % root.count)
					...
					PathCurve {
						id: outerCurve
						property real radius: shape.index === 0 ? root.circleRadius : root.outerRadius - outerVal * root.scale
					...
					PathCurve {
						id: innerCurve
						property real radius: root.outerRadius - innerVal * root.scale
						x: root.width / 2 + radius * xMultiplier
						y: root.height / 2 + radius * yMultiplier
					}
